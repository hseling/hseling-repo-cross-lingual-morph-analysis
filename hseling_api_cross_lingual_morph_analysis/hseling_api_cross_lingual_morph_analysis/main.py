...
from hseling_api_cross_lingual_morph_analysis import boilerplate

from hseling_lib_cross_lingual_morph_analysis.process import process_data
from hseling_lib_cross_lingual_morph_analysis.query import query_data


app = Flask(__name__)
app.config['DEBUG'] = False
app.config['LOG_DIR'] = '/tmp/'
app.config.from_envvar('HSELING_API_CROSS_LINGUAL_MORPH_ANALYSIS_SETTINGS')
...
if not app.debug:
    ...
    file_handler = TimedRotatingFileHandler(os.path.join(app.config['LOG_DIR'], 'hseling_api_cross_lingual_morph_analysis.log'), 'midnight')
...
def healthz():
    ...
    return jsonify({"status": "ok", "message": "Application Cross-lingual morphological analysis"})
